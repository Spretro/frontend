...
export default function TrendingProducts() {
  ...
  useEffect(() => {
    fetch(
      "https://dummyjson.com/products?limit=20&select=id,title,price,discountPercentage,thumbnail,brand,rating,category"
    )
      .then((r) => r.json())
  ...
  return (
    <section className="py-12" style={{ background: "linear-gradient(180deg, #F9F8FF 0%, #FFFFFF 100%)" }}>
      <style>{`
        ...
        .trend-extra { animation: trendSlideUp 0.35s ease both; }
      `}</style>
      ...
      <div className="max-w-360 mx-auto px-4 md:px-8">
                  ...
                  return (
                    ...
                    >
                      <div className="relative aspect-3/4 overflow-hidden bg-gray-50">
                        ...
                        <span className="absolute top-3 left-3 text-white text-[9px] font-black px-2.5 py-1.5 rounded-full shadow-lg" style={{ background: tag.color }}>{tag.label}</span>
                        <span className="absolute top-3 right-3 bg-white/95 text-emerald-600 text-[9px] font-black px-2.5 py-1.5 rounded-full shadow-md">-{Math.round(p.discountPercentage)}%</span>
        ...
        <div className="md:hidden">
          {premiumHeader(() => setExpanded((e) => !e))}
          {loading ? (
            <div className="grid grid-cols-2 gap-3">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="rounded-3xl animate-pulse bg-gray-100" style={{ aspectRatio: "3/4" }} />
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-2 gap-3">
              {visible.map((product, i) => (
                <div key={product.id} className={i >= 4 ? "trend-extra" : ""} style={i >= 4 ? { animationDelay: `${(i - 4) * 0.06}s` } : {}}>
          ...
          {!loading && products.length > 4 && (
            <div className="mt-8 flex justify-center">
              <button
                ...
                style={{
                  background: expanded ? "transparent" : "#6A2CFF",
                  color: expanded ? "#6A2CFF" : "white",
                  border: "2px solid #6A2CFF",
                  boxShadow: expanded ? "none" : "0 6px 24px #6A2CFF40",
                }}
              >
                {expanded ? "Show Less" : "View All →"}
              </button>
