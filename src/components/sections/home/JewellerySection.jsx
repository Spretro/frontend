            ...
            {trendingStyles.map((item, i) => (
              <div key={i} className="shrink-0 cursor-pointer group fade-in-up" style={{ animationDelay: `${i * 0.07}s` }}>
                <div
                  ...
                  onMouseEnter={(e) => (e.currentTarget.style.boxShadow = "0 16px 48px rgba(106,44,255,0.22)")}
                  onMouseLeave={(e) => (e.currentTarget.style.boxShadow = "0 4px 20px rgba(106,44,255,0.08)")}
                >
                  <img src={item.image} alt={item.label} className="w-full h-full object-cover group-hover:scale-107 transition-transform duration-600" />
                  <div className="absolute inset-0 bg-linear-to-t from-black/70 via-transparent to-transparent" />
                  <span className="absolute bottom-4 left-0 right-0 text-center text-white font-black text-sm uppercase tracking-wide drop-shadow-lg">{item.label}</span>
                </div>
              </div>
            ))}
          </div>
