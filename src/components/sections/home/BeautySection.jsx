    ...
    <>
        ...
        <div className="max-w-360 mx-auto px-4 md:px-8">
          ...
          <div className="flex gap-4 overflow-x-auto scrollbar-hide pb-3">
            {beautyCategories.map((cat, i) => (
              <div key={i} className="shrink-0 cursor-pointer group fade-in-up" style={{ animationDelay: `${i * 0.08}s` }}>
                <div
                  ...
                  onMouseEnter={(e) => (e.currentTarget.style.boxShadow = `0 16px 48px ${cat.accent}88`)}
                  onMouseLeave={(e) => (e.currentTarget.style.boxShadow = `0 4px 24px ${cat.accent}44`)}
                >
                  <img src={cat.image} alt={cat.label} className="w-full h-full object-cover opacity-85 group-hover:opacity-100 group-hover:scale-105 transition-all duration-500" />
                  <div className="absolute inset-0 bg-linear-to-t from-black/60 via-transparent to-transparent" />
                  <div className="absolute top-4 left-4 right-4">
                    <span className="text-white font-black text-sm uppercase tracking-widest drop-shadow-lg">{cat.label}</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="md:hidden">
        <CategoryProductSection
          title="Glam Up In" titleAccent="Minutes"
          subtitle="100% Genuine · Your Favourites"
          accentColor="#F472B6"
          bg="#130820"
          darkMode
          apiUrl="https://dummyjson.com/products/category/beauty?limit=20&select=id,title,price,thumbnail,rating,discountPercentage,brand"
        />
      </div>
    </>
  );
